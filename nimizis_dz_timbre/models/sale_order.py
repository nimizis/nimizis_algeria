...
class TimbreSale(models.Model):
    ...
    @api.depends('order_line.tax_id', 'order_line.price_unit', 'if_timbre', 'amount_total', 'amount_untaxed', 'currency_id')
    def _compute_tax_totals(self):
        for order in self:
            ...
            tax_totals['formatted_amount_total'] = formatLang(order.env, order.amount_total,currency_obj=order.currency_id)
            tax_totals['amount_timbre'] = formatLang(order.env, order.amount_timbre,currency_obj=order.currency_id)
